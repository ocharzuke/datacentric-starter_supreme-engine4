...
async function importData() {
  try {
    ...
    const genres = JSON.parse(
      fs.readFileSync("./sample_data/genres-collection.json", "utf8")
    );
    const categories = JSON.parse(
      fs.readFileSync("./sample_data/categories-collection.json", "utf8")
    );
    const users = JSON.parse(
      fs.readFileSync("./sample_data/users-collection.json", "utf8")
    );
    const movies = JSON.parse(
      fs.readFileSync("./sample_data/movies-collection.json", "utf8")
    );
    ...
    function prepareDocuments(documents) {
      return documents.map((doc) => {
        ...
        if (doc._id && doc._id.$oid) {
          doc._id = new ObjectId(String(doc._id.$oid));
        }
        ...
        if (doc.genre && doc.genre._id && doc.genre._id.$oid) {
          doc.genre._id = new ObjectId(String(doc.genre._id.$oid));
        }

        if (doc.categories) {
          doc.categories = doc.categories.map((category) => {
            if (category._id && category._id.$oid) {
              category._id = new ObjectId(String(category._id.$oid));
            }
            return category;
          });
        }
    ...
    await db
      .collection("genres-collection")
      .drop()
      .catch(() =>
        console.log("Genres collection does not exist yet - creating new")
      );
    await db
      .collection("categories-collection")
      .drop()
      .catch(() =>
        console.log("Categories collection does not exist yet - creating new")
      );
    await db
      .collection("users-collection")
      .drop()
      .catch(() =>
        console.log("Users collection does not exist yet - creating new")
      );
    await db
      .collection("movies-collection")
      .drop()
      .catch(() =>
        console.log("Movies collection does not exist yet - creating new")
      );

    // Insert data
    const genresResult = await db
      .collection("genres-collection")
      .insertMany(prepareDocuments(genres));
    console.log(`${genresResult.insertedCount} genres inserted`);

    const categoriesResult = await db
      .collection("categories-collection")
      .insertMany(prepareDocuments(categories));
    console.log(`${categoriesResult.insertedCount} categories inserted`);

    const usersResult = await db
      .collection("users-collection")
      .insertMany(prepareDocuments(users));
    console.log(`${usersResult.insertedCount} users inserted`);

    const moviesResult = await db
      .collection("movies-collection")
      .insertMany(prepareDocuments(movies));
